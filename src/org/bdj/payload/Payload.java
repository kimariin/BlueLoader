...
public class Payload {
	public static void main(UITextConsole console) throws Exception {
		...
		console.add("ZIP dumper listening on port 9030");
		...
		while (!dumped) {
			...
			Payload.dumpDirectoryToZip(zip, app0, console);

			// Just class files, e.g. /dsm/app_base/org/bdj/MainXlet.class
			// File cwd = new File(".");
			// Payload.dumpDirectoryToZip(zip, cwd, console);

			// Just to show where the Blu-ray is mounted
			// File disc = new File("/disc");
			// Payload.dumpDirectoryToZip(zip, disc, console);
